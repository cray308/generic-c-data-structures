...
/**
 * Creates a new array as a copy of a built-in array data type.
 *
 * @param   id    ID used with gen_array.
 ...
 *   elements are removed. If n is greater than the current size, elements are appended to the
 *   array with a value of 0.
 *
 * @param   id     ID used with gen_array.
 * @param   a      Pointer to array.
...
/**
 * Inserts the new element BEFORE the specified index. Any elements after this index will be
 * shifted one position to the right. After insertion, the new element will be located at the
 * specified index.
 *
 * @param   id       ID used with gen_array.
 * @param   a        Pointer to array.
 ...
 */
#define array_insert(id, a, index, element) array_insert_##id(a, index, element)


/**
 * Inserts new elements BEFORE the specified index. Any elements after this index will be shifted
 * one position to the right. After insertion, the first of the new elements will be located at the
 * specified index.
 *
 * @param   id     ID used with gen_array.
 * @param   a      Pointer to array.
 ...
 *
 * @return         The index where the first item was inserted, or ARRAY_ERROR if there was an error.
 */
...
__DS_FUNC_PREFIX Array_##id *array_new_##id(void) {                                                          \
    Array_##id *a = __ds_calloc(1, sizeof(Array_##id));                                                      \
    array_reserve_##id(a, 8);                                                                                \
    return a;                                                                                                \
}                                                                                                            \
                                                                                                             \
...
__DS_FUNC_PREFIX_INL void array_shrink_to_fit_##id(Array_##id *a) {                                          \
    if (a->size == a->capacity || a->size == 0) return;                                                      \
                                                                                                             \
    t *tmp = __ds_realloc(a->arr, a->size * sizeof(t));                                                      \
    a->capacity = a->size;                                                                                   \
    a->arr = tmp;                                                                                            \
}                                                                                                            \
                                                                                                             \
...
__DS_FUNC_PREFIX Array_2d_##id *matrix_new_##id(int rows, int cols) {                                        \
    ...
    Array_2d_##id *m = __ds_malloc(sizeof(Array_2d_##id));                                                   \
    ...
    for (int i = 0; i < rows; ++i) {                                                                         \
        arr = array_new_repeatingValue_##id(0, cols);                                                        \
