...
void test_insert_element(void) {
    ...
    int a1[] = {50,40,30,20,10}, i, inserted = -1, *p1;
    char *a2[] = {"050","040","030","020","010"}, **p2;

    for (i = 0; i < 5; ++i) {
        p1 = uset_insert_withResult(int, si, a1[i], &inserted);
        assert(p1 && *p1 == a1[i]);
        assert(inserted);
        inserted = -1;
        p2 = uset_insert_withResult(str, ss, a2[i], &inserted);
        assert(p2 && streq(*p2, a2[i]));
        assert(inserted);
        inserted = -1;
    }
    ...
    p1 = uset_insert_withResult(int, si, 40, &inserted);
    assert(p1 && *p1 == 40);
    assert(!inserted);
    inserted = -1;
    p2 = uset_insert_withResult(str, ss, "040", &inserted);
    assert(p2 && streq(*p2, "040"));
    assert(!inserted);
    inserted = -1;
    p1 = uset_insert_withResult(int, si, 10, &inserted);
    assert(p1 && *p1 == 10);
    assert(!inserted);
    inserted = -1;
    p2 = uset_insert_withResult(str, ss, "010", &inserted);
    assert(p2 && streq(*p2, "010"));
    assert(!inserted);
    inserted = -1;
    p1 = uset_insert_withResult(int, si, 50, &inserted);
    assert(p1 && *p1 == 50);
    assert(!inserted);
    inserted = -1;
    p2 = uset_insert_withResult(str, ss, "050", &inserted);
    assert(p2 && streq(*p2, "050"));
    assert(!inserted);
    compare_ints(si, c1, 5);
    compare_strs(ss, c2, 5);
    uset_free(int, si);
...
void test_remove_value(void) {
    ...
    for (i = 0; i < 8; ++i) {
        assert(uset_remove(int, si, removedInts[i]) == removed[i]);
        assert(uset_remove(str, ss, removedStrs[i]) == removed[i]);
    }
...
void test_set_load_factor(void) {
    ...
    uset_set_load_factor(str, ss, 24);
    assert(uset_max_load_factor(si) == 75);
    assert(uset_max_load_factor(ss) == 75);
    uset_set_load_factor(int, si, 125);
    uset_set_load_factor(str, ss, 125);
    assert(uset_max_load_factor(si) == 75);
    assert(uset_max_load_factor(ss) == 75);
    ...
    assert(uset_max_load_factor(si) == 75);
    assert(uset_max_load_factor(ss) == 75);
    uset_set_load_factor(int, si, 100);
    uset_set_load_factor(str, ss, 100);
    assert(uset_max_load_factor(si) == 100);
    assert(uset_max_load_factor(ss) == 100);
    compare_ints(si, c1, 50);
    compare_strs(ss, c2, 50);
    uset_set_load_factor(int, si, 25);
    uset_set_load_factor(str, ss, 25);
    assert(uset_max_load_factor(si) == 25);
    assert(uset_max_load_factor(ss) == 25);
    compare_ints(si, c1, 50);
    compare_strs(ss, c2, 50);
    uset_free(int, si);
    uset_free(str, ss);
